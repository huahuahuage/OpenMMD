...
def show_anim_curves(anim_dict, _plt):
    openpose_res_list = list(anim_dict.values())
    len_openpose_bone = 36
    if len(openpose_res_list) > 0:
        len_openpose_bone = len(openpose_res_list[0])
    val = np.array(openpose_res_list)
    for o in range(0,len_openpose_bone,2):
        x = val[:,o]
        y = val[:,o+1]
        logger.debug("x")
        logger.debug(x)
...
def read_openpose_json(now_str, idx, subdir, smooth=True, *args):
    ...
    for file_name in json_files:
        ...
        if int(frame_indx[0]) <= 0:
            ...
            # 将第一帧的数据放入临时数据中 idx=0(猜测,懒得看代码了)
            _tmp_data = data["people"][idx]["pose_keypoints_2d"]
            _tmp_points_len = int(len(data["people"][idx]["pose_keypoints_2d"])*2/3)
        else:
            # 前フレームと一番近い人物データを採用する
            # 记录上一帧的序列号,放入past_xy中
            ...
            # _tmp_points = [[0],[0],...,[0]] len=36
            # _tmp_points = [[0 for i in range(target_num)] for j in range(36)]
            # print("第{}帧长度{}".format(frame_indx[0],_tmp_points_len))
            # print(len(data["people"][0]["pose_keypoints_2d"]))
            _tmp_points = [[0 for i in range(target_num)] for j in range(_tmp_points_len)]
            # print(_tmp_points)

            # logger.debug("_past_tmp_points")
            # logger.debug(_past_tmp_points)

            for _data_idx in range(target_num):
...
def main():
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        for n, (frame, xy) in tqdm(enumerate(smoothed.items())):
            # print((frame, xy))
            ...
            # 将上面注释的代码替换成下面的
            # joints_array = np.array(xy)
            # joints_array.reshape(1,len(xy))
...
def run(dropout, epochs, load, gif_fps, verbose, openpose):
    openpose_output_dir = FLAGS.openpose
    ...
    FLAGS.evaluateActionWise = True

    FLAGS.load = load
    FLAGS.epochs = epochs
    FLAGS.gif_fps = gif_fps
    FLAGS.dropout = dropout
    FLAGS.verbose = verbose
    FLAGS.openpose = openpose
