...
def predict_video(model_path, video_path, baseline_path, interval, smoothed_2d):
    ...
    logger.info("Done!!深度推定結果: {0}".format(subdir))
...
def run(video_path, baseline_path, interval, verbose):
    logger.setLevel(level[verbose])
    # 関節二次元データを取得
    smoothed_2d = load_smoothed_2d("{0}/smoothed.txt".format(baseline_path))
    # 間隔は1以上の整数
    interval = interval if interval > 0 else 1
    # Predict the image
    model_path = os.path.join(os.getcwd(), "data/NYU_FCRN.ckpt")
    predict_video(model_path, video_path, baseline_path, interval, smoothed_2d)
