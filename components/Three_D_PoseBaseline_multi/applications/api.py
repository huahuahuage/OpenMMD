...
def run(target, verbose, centerxy, centerz, xangle, ddecimation, mdecimation, idecimation, alignment, legik, heelpos):
    ...
    is_alignment = True if alignment == 1 else False
    is_ik = True if legik == 1 else False

    # 入力と出力のファイル名は固定
    position_file = target + "/pos.txt"
    smoothed_file = target + "/smoothed.txt"
    depth_file = target + "/depth.txt"
    suffix = ""
    if mdecimation == 0 and idecimation == 0 and ddecimation == 0:
        suffix = "_間引きなし"
    ...
    vmd_file = "{0}/output_{1:%Y%m%d_%H%M%S}{2}.vmd".format(target, datetime.datetime.now(), suffix)

    # ログレベル設定
    logger.setLevel(level[verbose])

    position_multi_file_to_vmd(position_file, vmd_file, smoothed_file, bone, depth_file, upright - 1, centerxy, centerz, xangle, mdecimation, idecimation, ddecimation, is_alignment, is_ik, heelpos)
